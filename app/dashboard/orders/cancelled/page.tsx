...
export default function CancelledOrdersPage() {
  ...
  const { data: orders, isLoading, error, refetch } = useOrdersByStatus(storeId ?? '', OrderStatus.CANCELLED);

  if (!storeId || isLoading) return <LoadingState message="Loading cancelled orders..." />;
  if (error) return <ErrorState title="Failed to load cancelled orders" onRetry={refetch} />;
  if (!orders?.length) return <EmptyState title="No cancelled orders" description="Cancelled orders will appear here." />;

  return (
    <div className="p-6 space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Cancelled Orders</h1>
        <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">Review orders that were cancelled.</p>
      </div>
