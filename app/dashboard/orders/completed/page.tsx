...
export default function CompletedOrdersPage() {
  ...
  const { data: orders, isLoading, error, refetch } = useOrdersByStatus(storeId ?? '', OrderStatus.DELIVERED);

  if (!storeId || isLoading) return <LoadingState message="Loading completed orders..." />;
  if (error) return <ErrorState title="Failed to load completed orders" onRetry={refetch} />;
  if (!orders?.length) return <EmptyState title="No completed orders" description="Delivered orders will appear here." />;

  return (
    <div className="p-6 space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Completed Orders</h1>
        <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">Review delivered and completed orders.</p>
      </div>
